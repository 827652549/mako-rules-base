...
按以下 4 段结构生成 PRD，写入 Linear 评论（使用折叠区块格式）：
...
基于 PRD，产出用户体验方案，写入 Linear 评论（使用折叠区块格式）：
...
基于 PRD + UX + UI 三份产物，产出 TRD 和 Task 拆分。TRD 写入 Linear 评论（使用折叠区块格式），Task 拆分写入另一条评论（使用折叠区块格式）。
...
直接产出 TRD，写入 Linear 评论（使用折叠区块格式）：
...
然后产出 Task 拆分 JSON（同 2d 格式），写入 Linear 评论（使用折叠区块格式）。
...
在 Linear 评论中落库（均使用折叠区块格式）：

**分支 A（需求）**：
1. PRD（一条评论，`+++ 📋 PRD Agent | 需求分析总结 | 触发：Human 创建 issue 后自动执行`）
2. UX 方案（一条评论，`+++ 🎨 UX Agent | 用户体验方案 | 触发：PRD 完成后自动执行`）
3. UI 设计规范（一条评论，`+++ 🖌️ UI Agent | 视觉设计规范 | 触发：UX 方案完成后自动执行`）
4. TRD（一条评论，`+++ 📋 TRD Agent | 技术方案 | 触发：PRD/UX/UI 完成后自动执行`）
5. Task 拆分 JSON（一条评论，`+++ 📋 Task Breakdown | 任务拆分 | 触发：TRD 完成后自动执行`）+ 对应的 Linear 子任务

**分支 B（技改）**：
1. TRD（一条评论，`+++ 📋 TRD Agent | 技术方案 | 触发：Human 创建 issue 后自动执行`）
2. Task 拆分 JSON（一条评论，`+++ 📋 Task Breakdown | 任务拆分 | 触发：TRD 完成后自动执行`）+ 对应的 Linear 子任务
